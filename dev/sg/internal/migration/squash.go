...
func Squash(database db.Database, commit string) error {
	...
	contents := map[string]string{
		...
		unprivilegedFiles.MetadataFile: "name: 'squashed migrations'\n",
	}
	if privilegedUpMigration != "" {
		...
		contents[privilegedFiles.MetadataFile] = "name: 'squashed migrations (privileged)'\nprivileged: true\n"

		// Update new (unprivileged) root to declare the new privileged root as its parent
		contents[unprivilegedFiles.MetadataFile] = fmt.Sprintf("name: 'squashed migrations (unprivileged)'\nparent: %d\n", privilegedRoot)
	}
